...
def test_slerp_is_unit_norm(q_pair, t):
    ...
    assert np.allclose(np.linalg.norm(Q_slerp), 1.0, atol=1e-12)
...
def test_slerp_endpoints(q_pair):
    ...
    q0, q1 = q_pair
    S0 = slerp(q0, q1, 0)
    assert np.allclose(np.abs(np.dot(q0, S0)), 1.0, atol=1e-12)

    S1 = slerp(q0, q1, 1)
    assert np.allclose(np.abs(np.dot(q1, S1)), 1.0, atol=1e-12)
